...
def pst(piece_type, sqr, gamephase):
    mg = (100 - gamephase) / 100
    eg = gamephase / 100
    # flip board horizontally for black pieces
    sqrBlack = sqr + 7 - 2*(sqr % 8)
    if piece_type == 'P':
...
# mid-game callers
def pawn_map_mg(sqr):
    return pawn_map_mgt[sqr]
def knight_map_mg(sqr):
    return knight_map_mgt[sqr]
def bishop_map_mg(sqr):
    return bishop_map_mgt[sqr]
def rook_map_mg(sqr):
    return rook_map_mgt[sqr]
def queen_map_mg(sqr):
    return queen_map_mgt[sqr]
def king_map_mg(sqr):
    return king_map_mgt[sqr]

# end-game callers
def pawn_map_eg(sqr):
    return pawn_map_egt[sqr]
def knight_map_eg(sqr):
    return knight_map_egt[sqr]
def bishop_map_eg(sqr):
    return bishop_map_egt[sqr]
def rook_map_eg(sqr):
    return rook_map_egt[sqr]
def queen_map_eg(sqr):
    return queen_map_egt[sqr]
def king_map_eg(sqr):
    return king_map_egt[sqr]
...
# mid-game tables
                ...
                #  a    b    c    d    e    f    g    h
pawn_map_mgt = [   0,   0,   0,   0,   0,   0,   0,   0,  # 1
...
knight_map_mgt = [-105, -21, -58, -33, -17, -28, -19, -23,
...
bishop_map_mgt = [-33, -3, -14, -21, -13, -12, -39, -21,
              4, 15, 16, 0, 7, 21, 33, 1,
...
rook_map_mgt = [-19, -13, 1, 17, 16, 7, -37, -26,
...
queen_map_mgt = [-1, -18, -9, 10, -15, -25, -31, -50,
             -35, -8, 11, 2, 8, 15, -3, 1,
             -14, 2, -11, -2, -5, 2, 14, 5,
             -9, -26, -9, -10, -2, -4, 3, -3,
